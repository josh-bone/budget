...
def fetch_cells(service, spreadsheet_id: str, sheet: str, cell_refs: list[str]) -> dict[str, str]:
...
def print_section(section_name: str, labels: dict[str, str], cell_values: dict[str, str]):
    print(f"\n  {section_name.upper()}")
    rows = [[label, cell_ref, cell_values.get(cell_ref, "")] for label, cell_ref in labels.items()]
    print(tabulate(rows, headers=["Label", "Cell", "Value"], tablefmt="rounded_outline"))
